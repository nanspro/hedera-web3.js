...
window.onload = function() {
  ...
  hash.accountFiles = (accountId, params = null) => {
    return APIGetRequest(getUrl(`accounts/${accountId}`, params));
  };
  ...
  hash.deploySmartContract = (data, cb) => {
    ...
    hederaTag.setAttribute("data-transactionfee", data.transactionfee || "");
    hederaTag.setAttribute("data-amount", data.amount || "");
    hederaTag.setAttribute(
      "data-expirationTime",
      data.expirationTime || 7890000000
    );
    contractDiv.appendChild(hederaTag);
    if (cb) {
      _callback = cb;
  ...
  hash.accountIdToEthAddress = accountId => {
    ...
    let etherAddress =
      defaultAddress.substr(0, remainingCount) +
      etherAddressRaw +
      defaultAddress.substr(remainingCount + etherAddressRaw.length);
    return etherAddress;
  };
  ...
  document.addEventListener("DOMNodeInserted", () => {
    ...
    if (banner) {
      ...
      let smartContractTrigger = banner.getAttribute(
        "data-smart-contract-trigger"
      );
      console.log(smartContractTrigger);
      if (smartContractTrigger === "true") {
        banner.style.display = "none";
        console.log(
          `chrome-extension://${chromeExtensionId}/html/smart-contract-details.html?sender_url=${window.location.origin}`
        );
        let popupWindow = window.open(
          `chrome-extension://${chromeExtensionId}/html/smart-contract-details.html?sender_url=${window.location.origin}`,
          "extension_popup",
          "height=520,width=350,status=1,scrollbars=1,resizable=no"
        );
      ...
      let microPaymentTrigger = banner.getAttribute(
        "data-crypto-transfer-trigger"
      );
      if (microPaymentTrigger === "true") {
        banner.style.display = "none";
        window.open(
          `chrome-extension://${chromeExtensionId}/html/crypto-transfer-details.html?sender_url=${window.location.origin}`,
          "extension_popup",
          "height=520,width=350,status=1,scrollbars=1,resizable=no"
        );
        return;
    ...
    if (event.data.type && event.origin === window.location.origin) {
      ...
      switch (event.data.type) {
        case "account-connect-deny":
          window.hash.onConnectAccountError &&
            window.hash.onConnectAccountError(event.data.res);
          _callback && _callback(event.data.res, null);
          _reject && _reject(event.data.res);
          break;
        case "account-connect-success":
          window.hash.onConnectAccountSuccess &&
            window.hash.onConnectAccountSuccess(event.data.res);
          _callback && _callback(null, event.data.res);
          _resolve && _resolve(event.data.res);
          break;
        case "smart-contract-deny":
          window.hash.onSmartContractError &&
            window.hash.onSmartContractError(event.data.res);
          _callback && _callback(event.data.res, null);
          _reject && _reject(event.data.res);
          break;
        case "smart-contract-success":
          window.hash.onSmartContractSuccess &&
            window.hash.onSmartContractSuccess(event.data.res);
          _callback && _callback(null, event.data.res);
          _resolve && _resolve(event.data.res);
          break;
        case "micro-payment-deny":
          window.hash.onCryptoTransferError &&
            window.hash.onCryptoTransferError(event.data.res);
          _callback && _callback(event.data.res);
          _reject && _reject(event.data.res);
          break;
        case "micro-payment-success":
          window.hash.onCryptoTransferSuccess &&
            window.hash.onCryptoTransferSuccess(event.data.res);
          _callback && _callback(null, event.data.res);
          _resolve && _resolve(event.data.res);
          break;
        case "hedera-balance-success":
          window.hash.onCheckAccountBalance &&
            window.hash.onCheckAccountBalance(event.data.res);
          _callback && _callback(null, event.data.res);
          _resolve && _resolve(event.data.res);
          break;
        ...
        case "smart-contract-deploy-success":
          window.hash.onSmartContractDeploySuccess &&
            window.hash.onSmartContractDeploySuccess(event.data.res);
          _callback && _callback(null, event.data.res);
          _resolve && _resolve(event.data.res);
          break;
        case "smart-contract-deploy-deny":
          window.hash.onSmartContractDeployError &&
            window.hash.onSmartContractDeployError(event.data.res);
          _callback && _callback(event.data.res);
          _reject && _reject(event.data.res);
          break;
      }
